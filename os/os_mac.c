...
OS_linux_state *os_mac_state;

b32 os_sleep_ms(u32 ms);
Str8 os_file_name_from_path(Str8 path);
Date_time os_mac_date_time_from_tm(struct tm in, u32 msec);
struct tm os_mac_tm_from_date_time(Date_time dt);
Dense_time os_mac_dense_time_from_timespec(struct timespec in);
OS_file_iter* os_file_iter_begin(Arena *a, Str8 path, OS_file_iter_flags flags);
File_properties os_mac_file_properties_from_stat(struct stat *s);
...
OS_linux_entity* os_mac_entity_alloc(OS_linux_entity_kind kind);
void os_mac_entity_release(OS_linux_entity *entity);
...
func Date_time os_mac_date_time_from_tm(struct tm in, u32 msec) {
  Date_time dt = {0};
  dt.sec  = in.tm_sec;
  dt.min  = in.tm_min;
  dt.hour = in.tm_hour;
...
func struct tm os_mac_tm_from_date_time(Date_time dt) {
  struct tm result = {0};
...
func Dense_time os_mac_dense_time_from_timespec(struct timespec in) {
  Dense_time result = 0;
  {
    struct tm tm_time = {0};
    gmtime_r(&in.tv_sec, &tm_time);
    Date_time date_time = os_mac_date_time_from_tm(tm_time, in.tv_nsec/(MILLION(1)));
...
/* stat timespec helpers for mac */
static inline struct timespec st_mtime_ts(const struct stat *s) {
  return s->st_mtimespec;
}
static inline struct timespec st_birth_ts(const struct stat *s) {
  return s->st_birthtimespec;
}

func File_properties os_mac_file_properties_from_stat(struct stat *s) {
  File_properties props = {0};
  props.size     = s->st_size;
  props.created  = os_mac_dense_time_from_timespec(st_birth_ts(s));
  props.modified = os_mac_dense_time_from_timespec(st_mtime_ts(s));
...
func OS_linux_entity* os_mac_entity_alloc(OS_linux_entity_kind kind) {
  OS_linux_entity *entity = 0;

  defer_loop(
      pthread_mutex_lock(&os_mac_state->entity_mutex),
      pthread_mutex_unlock(&os_mac_state->entity_mutex)
      )
  {
    entity = os_mac_state->entity_free_list;
    if(entity) {
      sll_stack_pop(os_mac_state->entity_free_list);
    } else {
      entity = push_struct_no_zero(os_mac_state->entity_arena, OS_linux_entity);
    }
  }
...
func void os_mac_entity_release(OS_linux_entity *entity) {
  ...
  {
    sll_stack_push(os_mac_state->entity_free_list, entity);
  }

}

void* os_mac_thread_entry_point(void *ptr) {
...
func OS_handle os_thread_launch(OS_thread_func *fn, void *ptr, void *params) {
  ...
  OS_linux_entity *entity = os_mac_entity_alloc(OS_LINUX_ENTITY_THREAD);

  entity->e.thread.fn = fn;
  entity->e.thread.ptr = ptr;

  int pthread_result = pthread_create(&entity->e.thread.handle, 0, os_mac_thread_entry_point, (void*)entity);
  if(pthread_result != 0) {
...
func void os_thread_detach(OS_handle handle) {
  ...
  OS_linux_entity *entity = (OS_linux_entity*)handle.hand[0];
  /* actually detach the pthread to avoid resource leak */
  pthread_detach(entity->e.thread.handle);
  os_mac_entity_release(entity);
}

func OS_handle os_mutex_alloc(void) {
  OS_linux_entity *entity = os_mac_entity_alloc(OS_LINUX_ENTITY_MUTEX);
...
func OS_handle os_rw_mutex_alloc(void) {
  OS_linux_entity *entity = os_mac_entity_alloc(OS_LINUX_ENTITY_RWMUTEX);
...
func OS_handle os_cond_var_alloc(void) {
  OS_linux_entity *entity = os_mac_entity_alloc(OS_LINUX_ENTITY_CONDITION_VARIABLE);
...
func OS_handle os_semaphore_alloc(u32 initial_count, u32 max_count, Str8 name) {
  ...
  if(name.len > 0) {
    /* named semaphore requested */
    osx_sem_wrap *wrap = (osx_sem_wrap*)malloc(sizeof(*wrap));
    memset(wrap, 0, sizeof(*wrap));
    /* ensure leading '/'
       copy up to space-1 bytes */
    size_t n = (size_t)MIN((size_t)name.len, sizeof(wrap->name_buf)-2);
    wrap->name_buf[0] = '/';
    memcpy(&wrap->name_buf[1], name.s, n);
    wrap->name_buf[1+n] = '\0';
    wrap->unlink_on_release = 1; /* assume caller wants it cleaned */
    sem_t *s = sem_open(wrap->name_buf, O_CREAT | O_EXCL, 0644, initial_count);
    if(s == SEM_FAILED) {
      /* try open without O_CREAT|O_EXCL */
      s = sem_open(wrap->name_buf, 0);
      if(s == SEM_FAILED) {
        free(wrap);
        return result;
      }
    ...
    wrap->sem = s;
    result.hand[0] = (u64)wrap;
  } else {
    /* generate a private named semaphore */
    osx_sem_wrap *wrap = (osx_sem_wrap*)malloc(sizeof(*wrap));
    memset(wrap, 0, sizeof(*wrap));
    snprintf(wrap->name_buf, sizeof(wrap->name_buf), "/sem_%d_%llu", getpid(), (unsigned long long)clock());
    wrap->unlink_on_release = 1;
    sem_t *s = sem_open(wrap->name_buf, O_CREAT | O_EXCL, 0644, initial_count);
    if(s == SEM_FAILED) {
      free(wrap);
      return result;
...
func void  os_semaphore_release(OS_handle semaphore) {
  if(semaphore.hand[0] == 0) return;
  osx_sem_wrap *wrap = (osx_sem_wrap*)semaphore.hand[0];
  if(wrap->sem) sem_close(wrap->sem);
  if(wrap->unlink_on_release && wrap->name_buf[0]) sem_unlink(wrap->name_buf);
  free(wrap);
}
...
func OS_handle os_semaphore_open(Str8 name) {
  ...
  osx_sem_wrap *wrap = (osx_sem_wrap*)malloc(sizeof(*wrap));
  memset(wrap, 0, sizeof(*wrap));
  size_t n = (size_t)MIN((size_t)name.len, sizeof(wrap->name_buf)-1);
  if(name.s[0] != '/') {
    wrap->name_buf[0] = '/';
    memcpy(&wrap->name_buf[1], name.s, n);
    wrap->name_buf[1+n] = '\0';
  } else {
    memcpy(wrap->name_buf, name.s, n);
    wrap->name_buf[n] = '\0';
  }
  sem_t *s = sem_open(wrap->name_buf, 0);
  if(s == SEM_FAILED) {
    free(wrap);
    return result;
  }
  wrap->sem = s;
  wrap->unlink_on_release = 0;
  result.hand[0] = (u64)wrap;
  return result;
}
...
func b32 os_semaphore_take(OS_handle semaphore, u64 end_time_usec) {
  ASSERT_ALWAYS(end_time_usec == MAX_U64 || semaphore.hand[0] != 0);
  osx_sem_wrap *wrap = (osx_sem_wrap*)semaphore.hand[0];
  if(!wrap || !wrap->sem) return 0;

  if(end_time_usec == MAX_U64) {
    for(;;) {
      int r = sem_wait(wrap->sem);
      if(r == 0) break;
  ...
  struct timespec ts;
  ts.tv_sec = end_time_usec / MILLION(1);
  ts.tv_nsec = (long)((end_time_usec % MILLION(1)) * 1000ULL);
  for(;;) {
    int err = sem_timedwait(wrap->sem, &ts);
    if(err == 0) return 1;
    if(errno == EINTR) continue;
...
func void* os_state_get_selected(void) {
  return (void*)os_mac_state;
}

func void os_state_select(void *state) {
  os_mac_state = (OS_linux_state*)state;
}

func void* os_state_alloc(void) {
  {
    Arena *linux_state_arena = arena_alloc(KB(8));
    os_mac_state = push_struct(linux_state_arena, OS_linux_state);
    os_mac_state->arena = linux_state_arena;
    os_mac_state->entity_arena = arena_alloc(KB(16));
    pthread_mutex_init(&os_mac_state->entity_mutex, 0);
  }

  {
    OS_system_info *info = &os_mac_state->system_info;
  ...
  {
    OS_process_info *info = &os_mac_state->process_info;
    info->pid = (u32)getpid();
  }
...

  {
    ...
    if(got_final_result && size > 0) {
      ...
      info->machine_name.s = push_array_no_zero(os_mac_state->arena, u8, info->machine_name.len + 1);
  ...
  /* grab dynamically allocated process info */
  {
    ...
    /* binary path using _NSGetExecutablePath */
    {
      uint32_t cap = 0;
      _NSGetExecutablePath(NULL, &cap); /* get required size */
      char *buf = (char*)push_array_no_zero(scratch.arena, char, cap + 1);
      if(_NSGetExecutablePath(buf, &cap) == 0) {
        char resolved[PATH_MAX];
        if(realpath(buf, resolved)) {
          Str8 full_name = { .s = (u8*)resolved, .len = (s64)memory_strlen(resolved) };
          Str8 name_chopped = str8_chop_last_slash(full_name);
          info->binary_path = str8_copy(os_mac_state->arena, name_chopped);
        }
      }
    }
    ...
    /* initial directory */
    {
      info->initial_path = os_get_current_dir(os_mac_state->arena);
    }
    ...
    /* home directory */
    {
      ...
      if(home) {
        ...
        info->user_program_data_path = (Str8){0};
      }
