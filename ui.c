...
func void ui_draw(void) {
  ...
  for(UI_box *box = ui_state->root; box;) {
...

    {
      ...
      if(box->flags & UI_BOX_FLAG_DRAW_SIDE_LEFT) {
        draw_side = 1;
        start_pos.x = box->final_rect_min[0];
        start_pos.y = box->final_rect_min[1];
        end_pos.x = box->final_rect_min[0];
        end_pos.y = box->final_rect_max[1];
      }

      if(box->flags & UI_BOX_FLAG_DRAW_SIDE_RIGHT) {
        draw_side = 1;
        start_pos.x = box->final_rect_max[0];
        start_pos.y = box->final_rect_min[1];
        end_pos.x = box->final_rect_max[0];
        end_pos.y = box->final_rect_max[1];
      }

      if(box->flags & UI_BOX_FLAG_DRAW_SIDE_TOP) {
        draw_side = 1;
        start_pos.x = box->final_rect_min[0];
        start_pos.y = box->final_rect_min[1];
        end_pos.x = box->final_rect_max[0];
        end_pos.y = box->final_rect_min[1];
      }

      if(box->flags & UI_BOX_FLAG_DRAW_SIDE_BOTTOM) {
        draw_side = 1;
        start_pos.x = box->final_rect_min[0];
        start_pos.y = box->final_rect_max[1];
        end_pos.x = box->final_rect_max[0];
        end_pos.y = box->final_rect_max[1];
      }

      if(draw_side) {
        f32 thickness = box->border_size;
        Color color = box->border_color;
