...
func Ctoken_slice lex_ctoken(Arena *arena, Str8 src) {
  ...
  Str8 begin_comment_single_line = str8_lit("//");
  Str8 begin_comment_multi_line = str8_lit("/*");
  Str8 end_comment_multi_line = str8_lit("*/");
  ...
  for(s64 pos = 0; pos < src.len;) {
    ...
    if(str8_starts_with(pos_str, begin_comment_single_line)) {
    ...
    if(str8_starts_with(pos_str, begin_comment_multi_line)) {
      s64 end_comment = str8_find(pos_str, end_comment_multi_line);
      end_comment += end_comment_multi_line.len;

      Str8 comment_str = str8_slice(pos_str, 0, end_comment);
