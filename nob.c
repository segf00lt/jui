...
#define CLAY_HEADERS "third_party/clay/clay.h", "third_party/clay/clay_renderer_raylib.c"
...
int build_raylib(void) {
  ...
  typedef enum Raylib_build_kind {
    ...
    RAYLIB_BUILD_KIND_SHARED,
    RAYLIB_BUILD_KIND_DEBUG,
    RAYLIB_BUILD_KIND_WEB,
  } Raylib_build_kind;

  typedef struct Raylib_build {
  ...
  for(int build_i = 0; build_i < ARRLEN(raylib_builds); build_i++) {
    ...
    switch(raylib_builds[build_i].kind) {
      case RAYLIB_BUILD_KIND_STATIC:
        {
          Nob_Cmd cmd = {0};
